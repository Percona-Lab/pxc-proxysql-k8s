...
RUN apt-get update \
        && DEBIAN_FRONTEND=noninteractive apt-get install -y --force-yes \
                 percona-xtradb-cluster-57 curl pmm-client \
        && rm -rf /var/lib/apt/lists/* \
        && mv /etc/mysql/percona-xtradb-cluster.conf.d/mysqld.cnf /etc/mysql/my.cnf \
# comment out any "user" entires in the MySQL config ("docker-entrypoint.sh" or "--user" will handle user switching)
        && sed -ri 's/^user\s/#&/' /etc/mysql/my.cnf \
# purge and re-create /var/lib/mysql with appropriate ownership
        && rm -rf /var/lib/mysql && mkdir -p /var/lib/mysql /var/run/mysqld \
        && chown -R 1001:0 /var/lib/mysql /var/run/mysqld /etc/mysql/conf.d/ && chmod -R g+w /etc/mysql/conf.d/ \
